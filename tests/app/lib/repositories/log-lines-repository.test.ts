...
describe("LogLinesRepository", () => {
  ...
  it("inserts a log line", async () => {
    ...
    expect(db.calls[0].values).toEqual([`legacy-user:${sheet.id}`, 0, line.time, line.position, line.latitude, line.longitude, line.logNm, String(line.courseOverGround), String(line.compassCourse), String(line.waves), line.barometer, `${line.windDirection} ${line.windStrength}`.trim(), line.weather, line.sailNote, line.motorNote, line.windDirection, line.windStrength, line.windUnit, line.seaUnit, line.tide, line.tideUnit, line.moon, line.deviation, line.magneticCourse, line.variation, line.trueCourse, line.windDrift, line.courseThroughWater, line.currentDrift, line.courseOverGround, line.speedKn, line.sailSm, line.sailNote, line.motorSm, line.motorHours, line.motorNote, line.remarks]);
  });
});

function logLineRow(): LogLineRow {
  const { position, logNm, windDirection, windStrength, windUnit, waves, seaUnit, tideUnit, compassCourse, magneticCourse, trueCourse, windDrift, courseThroughWater, currentDrift, courseOverGround, speedKn, sailSm, sailNote, motorSm, motorHours, motorNote, ...rest } = line;
  return { ...rest, sheet_id: sheet.id, sort_order: 0, position_name: position, log_nm: logNm, wind_direction: windDirection, wind_strength: windStrength, wind_unit: windUnit, sea_state: waves, sea_unit: seaUnit, tide_unit: tideUnit, magnetic_course: compassCourse, magnetic_course_corrected: magneticCourse, true_course: trueCourse, drift_angle: windDrift, course_through_water: courseThroughWater, current_drift: currentDrift, course_over_ground: courseOverGround, speed_kn: speedKn, sail_sm: sailSm, sail_note: sailNote, motor_sm: motorSm, motor_hours: motorHours, motor_note: motorNote };
}
