...
describe("CrewRepository", () => {
  ...
  it("finds profiles ordered by primary flag and decrypted name", async () => {
    const profileRows = [
      profileRow("repository-user:zoe", "Zoe", 0),
      profileRow("repository-user:anna", "Anna", 0),
      profileRow("repository-user:captain", "Captain", 1),
    ];
    const db = new MockDatabase({ crew_members: profileRows });

    await expect(new CrewRepository(db).findProfiles("repository-user")).resolves.toMatchObject([
      { crew_member_id: "repository-user:captain", name: "Captain", is_primary: 1 },
      { crew_member_id: "repository-user:anna", name: "Anna", is_primary: 0 },
      { crew_member_id: "repository-user:zoe", name: "Zoe", is_primary: 0 },
    ]);
  ...
  it("finds all crew rows", async () => {
    const row: CrewMemberRow = { sheet_id: sheet.id, crew_member_id: "repository-user:luca-frei-swiss", sort_order: 0, ...crew, name: encryptCrewField("repository-user", "repository-user:luca-frei-swiss", "name", crew.name), nationality: encryptCrewField("repository-user", "repository-user:luca-frei-swiss", "nationality", crew.nationality), role: encryptCrewField("repository-user", "repository-user:luca-frei-swiss", "role", crew.role), address: encryptCrewField("repository-user", "repository-user:luca-frei-swiss", "address", crew.address ?? ""), certificate: encryptCrewField("repository-user", "repository-user:luca-frei-swiss", "certificate", crew.certificate ?? ""), embarkation_datetime: crew.embarkationDateTime, embarkation_position: crew.embarkationPosition, disembarkation_datetime: crew.disembarkationDateTime, disembarkation_position: crew.disembarkationPosition };
    ...
    await expect(new CrewRepository(db).findAll("repository-user")).resolves.toEqual([decryptedRow]);
  ...
  it("unwraps double-encrypted crew rows before returning logbook data", async () => {
    const crewMemberId = "repository-user:luca-frei-swiss";
    const encryptedName = encryptCrewField("repository-user", crewMemberId, "name", crew.name);
    const row: CrewMemberRow = {
      sheet_id: sheet.id,
      crew_member_id: crewMemberId,
      sort_order: 0,
      ...crew,
      name: encryptCrewField("repository-user", crewMemberId, "name", encryptedName),
      nationality: encryptCrewField("repository-user", crewMemberId, "nationality", crew.nationality),
      role: encryptCrewField("repository-user", crewMemberId, "role", crew.role),
      address: encryptCrewField("repository-user", crewMemberId, "address", crew.address ?? ""),
      certificate: encryptCrewField("repository-user", crewMemberId, "certificate", crew.certificate ?? ""),
    ...
    await expect(new CrewRepository(db).findAll("repository-user")).resolves.toMatchObject([{ name: crew.name }]);
  });
  ...
  it("stores plaintext when a submitted crew field already contains an encrypted value", async () => {
    ...
    await new CrewRepository(db).insert(sheet.id, 0, { ...crew, name: encryptedName }, "repository-user");
    ...
    expect(decryptCrewField("repository-user", crewMemberId, "name", storedName)).toBe(crew.name);
  });

  it("deletes all crew rows", async () => {
    const db = new MockDatabase();

    await new CrewRepository(db).deleteAll("repository-user");
    expect(db.calls).toEqual([
      { sql: "delete from sheet_crew_members where sheet_id in (select id from log_sheets where owner_id = $1)", values: ["repository-user"] },
      { sql: "delete from crew_members where owner_id = $1", values: ["repository-user"] },
    ]);
  });
  ...
  it("inserts a crew member", async () => {
    ...
    await new CrewRepository(db).insert(sheet.id, 0, crew, "repository-user");

    expect(db.calls[0].sql).toContain("insert into crew_members");
    expect(db.calls[0].values?.[0]).toBe("repository-user:luca-frei-swiss");
    expect(db.calls[0].values?.slice(6)).toEqual([crew.isPrimary ? 1 : 0, null, null, null, null, "repository-user"]);
  ...
  it("decrypts crew profile image payloads with profile rows", async () => {
    const image = { data: "base64-crew-private-face", mimeType: "image/jpeg", width: 320, height: 240 };
    const row = {
      ...profileRow("repository-user:luca-frei-swiss", crew.name, 1),
      image_data: encryptCrewField("repository-user", "repository-user:luca-frei-swiss", "image_data", image.data),
    ...
    await expect(new CrewRepository(db).findProfiles("repository-user")).resolves.toEqual([expect.objectContaining({
...
function profileRow(id: string, name: string, isPrimary: number): CrewMemberRow {
  return {
    id,
    crew_member_id: id,
    name: encryptCrewField("repository-user", id, "name", name),
    nationality: encryptCrewField("repository-user", id, "nationality", ""),
    role: encryptCrewField("repository-user", id, "role", ""),
    address: encryptCrewField("repository-user", id, "address", ""),
    certificate: encryptCrewField("repository-user", id, "certificate", ""),
