...
describe("BoatsRepository", () => {
  it("finds all boat rows", async () => {
    const row: BoatRow = { ...boat, flag_state: boat.flagState, home_port: boat.homePort, deviation_table: boat.deviationTable };
  ...
  it("inserts a boat with typed master data", async () => {
    const db = new MockDatabase();
    ...
    expect(db.calls[0].values).toEqual([`repository-user:${boat.id}`, boat.archived ? 1 : 0, boat.name, boat.type, boat.registration, boat.flagState, boat.homePort, boat.owner, boat.dimensions, boat.manufacturer, boat.mmsi, boat.logfactor, JSON.stringify(boat.deviationTable), JSON.stringify(boat.windDriftTable ?? []), null, "repository-user"]);
  });
  ...
  it("maps loaded boat image metadata back to an image payload", () => {
    ...
    const row: BoatRow = { ...boat, flag_state: boat.flagState, home_port: boat.homePort, deviation_table: boat.deviationTable, image_data: image.data, image_mime_type: image.mimeType, image_width: image.width, image_height: image.height };
