...
describe("profile endpoint", () => {
  ...
  it("updates profile view preferences", async () => {
    ...
    const response = await PATCH(new Request("https://ultilog.test/api/profile", {
      method: "PATCH",
      body: JSON.stringify({ action: "preferences", preferences: { countryCode: "US", language: "de", windUnit: "kn", waterHeightUnit: "ft", temperatureUnit: "f", coordinateFormat: "dms", distanceDisplayUnit: "mi", defaultBoatId: "boat-1", defaultCrewMemberIds: ["crew-1"], theme: "dark", isNavSlim: true, showCourseConversionTable: false, showAvatarOnPrint: false } }),
    }));

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ preferences: { countryCode: "US", language: "de", windUnit: "kn", waterHeightUnit: "ft", temperatureUnit: "f", coordinateFormat: "dms", distanceDisplayUnit: "mi", defaultBoatId: "boat-1", defaultCrewMemberIds: ["crew-1"], theme: "dark", isNavSlim: true, showCourseConversionTable: false, showAvatarOnPrint: false }, theme: "dark", isNavSlim: true });
