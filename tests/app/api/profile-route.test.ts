...
describe("profile endpoint", () => {
  ...
  it("returns the signed-in user profile", async () => {
    ...
    mockedFindUserById.mockResolvedValueOnce({ id: "user-1", name: "User", email: "user@example.test", groups: [], onboardingCompletedTasks: ["create_first_boat"] });

    const response = await GET();

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ id: "user-1", name: "User", email: "user@example.test", groups: [], onboardingCompletedTasks: ["create_first_boat"] });
    expect(mockedFindUserById).toHaveBeenCalledWith("user-1");
  });

  it("updates the signed-in user's name", async () => {
    mockedAuth.mockResolvedValueOnce(session);
    mockedUpdateUserName.mockResolvedValueOnce({ id: "user-1", name: "Updated User", email: "user@example.test", groups: [], onboardingCompletedTasks: [] });
  ...
  it("updates the signed-in user's email", async () => {
    mockedAuth.mockResolvedValueOnce(session);
    mockedUpdateUserEmail.mockResolvedValueOnce({ id: "user-1", name: "User", email: "updated@example.test", groups: [], onboardingCompletedTasks: [] });
  ...
  it("updates manual onboarding completion", async () => {
    mockedAuth.mockResolvedValueOnce(session);
    mockedUpdateUserOnboardingCompletedTasks.mockResolvedValueOnce({ id: "user-1", name: "User", email: "user@example.test", groups: [], onboardingCompletedTasks: ["create_first_boat", "create_first_logsheet"] });
