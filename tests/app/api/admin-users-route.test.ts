...
const adminSession = { user: { id: "admin-user", name: "Admin", email: "admin@example.test", groups: ["admin"], onboardingCompletedTasks: [], theme: "light" as const, isNavSlim: false }, expires: "2099-01-01T00:00:00.000Z" };
...
describe("admin users endpoint", () => {
  ...
  it("lists users and known groups for admins", async () => {
    const listedUsers = [{ id: "demo", name: "Demo", email: "demo@ultilog.local", groups: ["demo"], onboardingCompletedTasks: [], theme: "light" as const, isNavSlim: false }];
    mockedAuth.mockResolvedValueOnce(adminSession);
  ...
  it("updates groups for admins", async () => {
    const updatedUser = { id: "demo", name: "Demo", email: "demo@ultilog.local", groups: ["demo", "reviewer"], onboardingCompletedTasks: [], theme: "light" as const, isNavSlim: false };
    mockedAuth.mockResolvedValueOnce(adminSession);
    mockedUserHasGroup.mockResolvedValueOnce(true);
