...
| `NEXT_PUBLIC_APP_URL` / `AUTH_URL` / `NEXTAUTH_URL` | Required for deployed password reset links. | Sets the public base URL used in password reset emails. `NEXT_PUBLIC_APP_URL` takes precedence; local development falls back to `http://localhost:3000`. |
