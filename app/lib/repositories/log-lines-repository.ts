...
export class LogLinesRepository {
  ...
  async insert(sheetId: string, sortOrder: number, line: LogLine) {
    await this.db.query(
      ...
      [scopedId((this.db as { ownerId?: string }).ownerId ?? "legacy-user", sheetId), sortOrder, line.time, line.position, line.latitude, line.longitude, line.logNm, line.course, line.magneticCourse, line.seaState, line.barometer, line.wind, line.weather, line.sails, line.engine, line.remarks],
    );
  }
