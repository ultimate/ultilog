...
type UserRow = Omit<AppUser, "avatar" | "hasUploadedAvatar" | "groups" | "onboardingCompletedTasks" | "hasReadCompliance" | "isNavSlim" | "countryCode" | "dateFormat" | "timeFormat" | "windUnit" | "waterHeightUnit" | "temperatureUnit" | "coordinateFormat" | "distanceDisplayUnit" | "defaultBoatId" | "defaultCrewMemberIds" | "showCourseConversionTable" | "showAvatarOnPrint" | "motionStationaryThresholdNm" | "technicalLogTemplate" | "emailVerified"> & { avatar_data: string | null; avatar_mime_type: string | null; password_hash: string; onboarding_completed_tasks: string; country_code: string; date_format: string; time_format: string; wind_unit: string; water_height_unit: string; temperature_unit: string; coordinate_format: string; distance_display_unit: string; default_boat_id: string; default_crew_member_ids: string; nav_slim: number | boolean; has_read_compliance: number | boolean; show_course_conversion_table: number | boolean; show_avatar_on_print: number | boolean; motion_stationary_threshold_nm: number | string | null; technical_log_template: string; email_verified_at: string | null };
...
function normalizeUserPreferences(input: Partial<Record<keyof UserPreferences, unknown>>, current?: UserRow, strict = false): UserPreferences {
  ...
  return {
    ...
    dateFormat: normalizeEnum(input.dateFormat, dateFormats, currentUser?.dateFormat ?? defaultDateFormat, "Date format", strict),
    timeFormat: normalizeEnum(input.timeFormat, timeFormats, currentUser?.timeFormat ?? defaultTimeFormat, "Time format", strict),
...
function toAppUser(user: UserRow, groups: string[]): AppUser {
  return {
    ...
    dateFormat: normalizeEnum(user.date_format, dateFormats, defaultDateFormat, "Date format"),
    timeFormat: normalizeEnum(user.time_format, timeFormats, defaultTimeFormat, "Time format"),
