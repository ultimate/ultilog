...
/** The comprehensive ISO 3166-1 alpha-2 catalog used by user profiles. */
export const countryFlagGroups = flagGroups.filter((group) => group.continent !== "Other");
export const supportedCountryCodes = new Set(
  countryFlagGroups.flatMap((group) => group.flags.map((flag) => flag.code)),
);
