...
export function ModuleTabs({ activeModule, onSelectModule, onOpenProfile, theme, onToggleTheme, userEmail, isNavSlim, onToggleNavSlim, onLogout, isLoggingOut }: ModuleTabsProps) {
  return (
    <aside className="app-sidebar" aria-label="Primary navigation">
      <div className="brand-row"><div className="brand-mark" aria-label="ultilog"><span className="sail-logo">◢</span><strong>ultilog</strong></div><button className="nav-slim-toggle" type="button" onClick={onToggleNavSlim} aria-label={isNavSlim ? "Expand menu labels" : "Collapse menu labels"}>{isNavSlim ? "»" : "«"}</button></div>
