...
import type { CrewForm, LogSheet, PersistedLogbook } from "../../../models/logbook";
import { defaultCrewForm } from "../forms";
import { modulePath } from "../persistence";
import { ManagerShell } from "../../managers/ManagerShell";

type CrewAssignment = { member: PersistedLogbook["crewMembers"][number]; sheets: { sheet: LogSheet; isSkipper: boolean }[] };
type CrewManagerPageProps = Record<string, any>;

export function CrewManagerPage(props: CrewManagerPageProps) {
  const { selectedCrewIndex, lastCrewIndex, setLastCrewIndex, setSelectedCrewIndex, selectCrew, pushAppPath, saveCrew, cancelCrewEdit, deleteSelectedCrew } = props;
