...
export function ProfilePage(props: ProfilePageProps) {
  const { logout, isLoggingOut, accountName, accountEmail, profileMessage, profileError, updateName, updateEmail, updatePassword, deleteAccount, selectCrew, navigate, theme } = props;
  ...
  const emailForm = props.emailForm as { email: string; currentPassword: string };
  const passwordForm = props.passwordForm as { currentPassword: string; newPassword: string; confirmPassword: string };
  const deleteForm = props.deleteForm as { currentPassword: string; confirmation: string };
  const setNameForm = props.setNameForm as Dispatch<SetStateAction<typeof nameForm>>;
  const setEmailForm = props.setEmailForm as Dispatch<SetStateAction<typeof emailForm>>;
  const setPasswordForm = props.setPasswordForm as Dispatch<SetStateAction<typeof passwordForm>>;
  const setDeleteForm = props.setDeleteForm as Dispatch<SetStateAction<typeof deleteForm>>;

  return <section className="profile-page module-panel" aria-label="Profile page">
