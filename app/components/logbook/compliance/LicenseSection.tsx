  ...
  return <section className="license-section" aria-labelledby="license-progress-title" aria-busy={loading}>
    ...
    <div className="tracked-license-list">{state.licenses.map((tracked) => {
      ...
      return <article className="tracked-license" key={license.id}>
        ...
        <section className="license-document"><p>{content.title}</p><dl className="source-metadata">{content.authority ? <div><dt>{t("compliance.authority")}</dt><dd>{content.authority}</dd></div> : null}{content.checkedAt ? <div><dt>{t("compliance.checkedAt")}</dt><dd>{content.checkedAt}</dd></div> : null}{content.effectiveFrom ? <div><dt>{t("compliance.effectiveFrom")}</dt><dd>{content.effectiveFrom}</dd></div> : null}</dl>{content.sections.map((section) => <section key={section.id}><h4>{section.heading}</h4>{section.citation ? <div className="legal-text" dangerouslySetInnerHTML={{ __html: section.citation }} /> : null}</section>)}<a href={content.sourceUrl} target="_blank" rel="noopener noreferrer">{t("compliance.officialSource")}</a></section>
        <section className="requirement-panel"><h4>{t("compliance.checklist")}</h4>{progress.map((item) => <article className={`requirement-row requirement-${item.verification}`} key={item.requirement.id}>
