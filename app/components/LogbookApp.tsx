...
export function LogbookApp({ userEmail }: { userEmail?: string }) {
  ...
  async function addCrewToActiveSheet(crewId: string) {
    ...
    if (!member) return;
    const nextLogbook = { ...logbookRef.current, sheets: logbookRef.current.sheets.map((sheet) => sheet.id === activeSheet.id && !sheet.crew.some((crew) => crew.id === crewId) ? { ...sheet, crew: [...sheet.crew, { ...member, embarkation: sheet.route.from, disembarkation: sheet.route.to }] } : sheet) };
  ...
  async function deleteSelectedCrew() {
    if (!selectedCrew || selectedCrew.isPrimary || crewAssignments.find((entry) => entry.member.id === selectedCrew.id)?.sheets.length) return;
    ...
    <main className="app-shell" data-theme={theme} data-nav={isNavSlim ? "slim" : "full"}>
      ...
      <section className="workspace module-workspace">
        ...
        {activeModule === "details" && <section className="sheet-detail" aria-labelledby="sheet-title">
          ...
          <div className="bottom-grid"><article className="info-card"><h3>Crew for this sheet</h3><label>Add crew member<select defaultValue="" onChange={(e) => { if (e.target.value) addCrewToActiveSheet(e.target.value); e.currentTarget.value = ""; }}><option value="">Select crew…</option>{logbook.crewMembers.filter((member) => !activeSheet.crew.some((crew) => crew.id === member.id)).map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label><ul className="stack-list">{activeSheet.crew.map((person, index) => <li key={person.id}><strong>{index === 0 ? "⭐ Skipper · " : ""}{person.name}</strong><span>{person.nationality} · {person.role}</span><small>{person.embarkation} → {person.disembarkation}</small><span><button type="button" className="edit-chip" disabled={index === 0} onClick={() => moveCrewOnActiveSheet(index, -1)}>↑</button><button type="button" className="edit-chip" disabled={index === activeSheet.crew.length - 1} onClick={() => moveCrewOnActiveSheet(index, 1)}>↓</button></span></li>)}</ul></article><article className="info-card"><h3>Watch & daily checks</h3><ul className="check-list">{[...activeSheet.watchPlan, ...activeSheet.technicalChecks].map((item) => <li key={item}>{item}</li>)}</ul></article></div>
