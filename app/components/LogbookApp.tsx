...
export function LogbookApp() {
  ...
  function saveCrew() {
    const crew = { ...crewForm };
    setLogbook((current) => ({ ...current, sheets: current.sheets.map((sheet) => sheet.id === activeSheet.id ? { ...sheet, crew: sheet.crew.map((candidate, index) => index === selectedCrewIndex ? crew : candidate) } : sheet) }));
  }
