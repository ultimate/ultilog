...
import { dateTimeLocalFromParts, dateTimeLocalFromStamp, routeStamp, routeStampFromDateTimeLocal, splitDateTimeLocal } from "./logbook/date-utils";
...
export function LogbookApp({ userId, userEmail, userName, userGroups = [] }: { userId?: string; userEmail?: string; userName?: string; userGroups?: string[] }) {
  ...
  async function saveSheet(event: FormEvent<HTMLFormElement>) {
    ...
    const initialCrew = [{ ...currentUserCrew, embarkationDateTime: dateTimeLocalFromStamp(route.departed), embarkationPosition: route.from, disembarkationDateTime: dateTimeLocalFromStamp(route.arrived), disembarkationPosition: route.to }];
  ...
  async function addCrewToActiveSheet(crewId: string) {
    ...
    if (!member) return;
    const nextLogbook = { ...logbookRef.current, sheets: logbookRef.current.sheets.map((sheet) => sheet.id === activeSheet.id && !sheet.crew.some((crew) => crew.id === crewId) ? { ...sheet, crew: [...sheet.crew, { ...member, embarkationDateTime: dateTimeLocalFromStamp(sheet.route.departed), embarkationPosition: sheet.route.from, disembarkationDateTime: dateTimeLocalFromStamp(sheet.route.arrived), disembarkationPosition: sheet.route.to }] } : sheet) };
    await saveLogbookNow(nextLogbook);
  }

  async function updateCrewAssignment(index: number, field: "embarkationDateTime" | "embarkationPosition" | "disembarkationDateTime" | "disembarkationPosition", value: string) {
    if (activeSheet.status === "Locked") return;
    const nextLogbook = { ...logbookRef.current, sheets: logbookRef.current.sheets.map((sheet) => {
      if (sheet.id !== activeSheet.id) return sheet;
  ...
  return (
    <main className="app-shell" data-theme={theme} data-nav={isNavSlim ? "slim" : "full"}>
      ...
      <section className="workspace module-workspace">
        {activeModule === "logbooks" && <LogbookListPage activeBoat={activeBoat} calculateSheetSummary={calculateSheetSummary} logbook={logbook} navigate={navigate} setActiveSheetId={setActiveSheetId} setEditingSheetId={setEditingSheetId} setSheetForm={setSheetForm} setShowNewSheet={setShowNewSheet} />}

        {activeModule === "details" && <LogbookDetailsPage isBackendReady={isBackendReady} showNewSheet={showNewSheet} editingSheetId={editingSheetId} saveSheet={saveSheet} sheetForm={sheetForm} setSheetForm={setSheetForm} logbook={logbook} setSelectedBoatId={setSelectedBoatId} setEditingBoatId={setEditingBoatId} setBoatForm={setBoatForm} setShowBoatManager={setShowBoatManager} navigate={navigate} cancelSheetEdit={cancelSheetEdit} activeSheet={activeSheet} renderInlineTextField={renderInlineTextField} isActiveSheetLocked={isActiveSheetLocked} updateActiveSheetStatus={updateActiveSheetStatus} renderInlineBoatField={renderInlineBoatField} activeBoat={activeBoat} renderInlineDateField={renderInlineDateField} activeSheetSummary={activeSheetSummary} setShowCourseColumns={setShowCourseColumns} showCourseColumns={showCourseColumns} startAddingLine={startAddingLine} showAddLine={showAddLine} lineForm={lineForm} setLineForm={setLineForm} saveLineFromFields={saveLineFromFields} editingLineIndex={editingLineIndex} cancelLineEdit={cancelLineEdit} startEditingLine={startEditingLine} updateCrewAssignment={updateCrewAssignment} moveCrewOnActiveSheet={moveCrewOnActiveSheet} deleteCrewFromActiveSheet={deleteCrewFromActiveSheet} addCrewToActiveSheet={addCrewToActiveSheet} />}

        {activeModule === "boats" && <BoatManagerPage logbook={logbook} selectedBoat={selectedBoat} setEditingBoatId={setEditingBoatId} setBoatForm={setBoatForm} setShowBoatManager={setShowBoatManager} showBoatManager={showBoatManager} boatForm={boatForm} saveBoat={saveBoat} editingBoatId={editingBoatId} cancelBoatEdit={cancelBoatEdit} deleteSelectedBoat={deleteSelectedBoat} setSelectedBoatId={setSelectedBoatId} pushAppPath={pushAppPath} />}

        {activeModule === "crew" && <CrewManagerPage selectedCrewIndex={selectedCrewIndex} lastCrewIndex={lastCrewIndex} setLastCrewIndex={setLastCrewIndex} setSelectedCrewIndex={setSelectedCrewIndex} selectCrew={selectCrew} pushAppPath={pushAppPath} saveCrew={saveCrew} cancelCrewEdit={cancelCrewEdit} deleteSelectedCrew={deleteSelectedCrew} crewForm={crewForm} setCrewForm={setCrewForm} crewAssignments={crewAssignments} logbook={logbook} />}


        {activeModule === "users" && <UserListPage mockSocialUsers={mockSocialUsers} />}

        {activeModule === "profile" && <ProfilePage logout={logout} isLoggingOut={isLoggingOut} accountName={accountName} accountEmail={accountEmail} userGroups={userGroups} profileMessage={profileMessage} profileError={profileError} updateName={updateName} updateEmail={updateEmail} updatePassword={updatePassword} deleteAccount={deleteAccount} selectCrew={selectCrew} navigate={navigate} theme={theme} logbook={logbook} activeBoat={activeBoat} nameForm={nameForm} setNameForm={setNameForm} emailForm={emailForm} setEmailForm={setEmailForm} passwordForm={passwordForm} setPasswordForm={setPasswordForm} deleteForm={deleteForm} setDeleteForm={setDeleteForm} />}
