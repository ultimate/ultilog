      ...
      <section className="workspace module-workspace">
        {activeModule === "logbooks" && <aside className="sidebar module-panel" aria-label="Log sheets">
          <div className="sidebar-header">
            ...
            <button type="button" onClick={() => { setEditingSheetId(null); setSheetForm(defaultSheetForm(activeBoat.id)); setShowNewSheet((show) => !show); }}>+ New sheet</button>
          </div>
          <button className="manager-toggle" type="button" onClick={() => { setEditingBoatId(null); setBoatForm(defaultBoatForm); setShowBoatManager((show) => !show); }}>+ New boat ({logbook.boats.length})</button>
        ...
        {activeModule === "details" && <section className="sheet-detail" aria-labelledby="sheet-title">
          {(showNewSheet || editingSheetId === activeSheet.id) ? (
            <form className="sheet-title-row inline-edit-card" onSubmit={saveSheet}>
              ...
              <div className="inline-edit-actions"><button type="submit">{editingSheetId ? "Save sheet" : "Create sheet"}</button><button type="button" className="ghost-button" onClick={cancelSheetEdit}>Cancel</button></div>
            </form>
          ) : (
            <>
          ...
          <div className="detail-grid"><article className="info-card">{(showBoatManager || editingBoatId === activeBoat.id) ? <form className="inline-edit-grid" onSubmit={saveBoat}><p className="eyebrow">{editingBoatId ? "Edit boat" : "New boat"}</p><label>Name<input required value={boatForm.name} onChange={(e) => setBoatForm({ ...boatForm, name: e.target.value })} /></label><label>Type<select value={boatForm.type} onChange={(e) => setBoatForm({ ...boatForm, type: e.target.value as BoatType })}><option>Sail</option><option>Motor</option></select></label><label>Registration<input value={boatForm.registration} onChange={(e) => setBoatForm({ ...boatForm, registration: e.target.value })} /></label><label>Flag state<input value={boatForm.flagState} onChange={(e) => setBoatForm({ ...boatForm, flagState: e.target.value })} /></label><label>Home port<input value={boatForm.homePort} onChange={(e) => setBoatForm({ ...boatForm, homePort: e.target.value })} /></label><label>Owner<input value={boatForm.owner} onChange={(e) => setBoatForm({ ...boatForm, owner: e.target.value })} /></label><label>Dimensions<input value={boatForm.dimensions} onChange={(e) => setBoatForm({ ...boatForm, dimensions: e.target.value })} /></label><label>Manufacturer<input value={boatForm.manufacturer} onChange={(e) => setBoatForm({ ...boatForm, manufacturer: e.target.value })} /></label><label>MMSI<input value={boatForm.mmsi} onChange={(e) => setBoatForm({ ...boatForm, mmsi: e.target.value })} /></label><label>Engine<input value={boatForm.engine} onChange={(e) => setBoatForm({ ...boatForm, engine: e.target.value })} /></label><label className="wide-field">Safety<textarea value={boatForm.safety} onChange={(e) => setBoatForm({ ...boatForm, safety: e.target.value })} /></label><div className="inline-edit-actions"><button type="submit">{editingBoatId ? "Save boat" : "Create boat"}</button><button type="button" className="ghost-button" onClick={cancelBoatEdit}>Cancel</button></div></form> : <><div className="card-title-row"><h3>Boat</h3><button type="button" className="edit-chip" onClick={() => startEditingBoat(activeBoat)}>Edit boat</button></div><dl><div><dt>Name</dt><dd>{activeBoat.name}</dd></div><div><dt>Type</dt><dd>{activeBoat.type}</dd></div><div><dt>Registration</dt><dd>{activeBoat.registration || "—"}</dd></div><div><dt>Flag / home port</dt><dd>{activeBoat.flagState || "—"} · {activeBoat.homePort || "—"}</dd></div><div><dt>Owner</dt><dd>{activeBoat.owner || "—"}</dd></div><div><dt>Ship data</dt><dd>{activeBoat.dimensions || "—"}</dd></div></dl></>}</article><article className="info-card"><h3>Skipper & ports</h3><dl><div><dt>Skipper</dt><dd>{activeSheet.skipper.name}</dd></div><div><dt>Address</dt><dd>{activeSheet.skipper.address}</dd></div><div><dt>Nationality</dt><dd>{activeSheet.skipper.nationality}</dd></div><div><dt>Certificate</dt><dd>{activeSheet.skipper.certificate}</dd></div><div><dt>Departure</dt><dd>{activeSheet.route.departed}</dd></div><div><dt>Arrival</dt><dd>{activeSheet.route.arrived}</dd></div></dl></article></div>
        ...
        {activeModule === "boats" && <section className="sheet-detail module-panel"><div className="detail-grid"><article className="info-card">{(showBoatManager || editingBoatId === activeBoat.id) ? <form className="inline-edit-grid" onSubmit={saveBoat}><p className="eyebrow">{editingBoatId ? "Edit boat" : "New boat"}</p><label>Name<input required value={boatForm.name} onChange={(e) => setBoatForm({ ...boatForm, name: e.target.value })} /></label><label>Type<select value={boatForm.type} onChange={(e) => setBoatForm({ ...boatForm, type: e.target.value as BoatType })}><option>Sail</option><option>Motor</option></select></label><label>Registration<input value={boatForm.registration} onChange={(e) => setBoatForm({ ...boatForm, registration: e.target.value })} /></label><label>Flag state<input value={boatForm.flagState} onChange={(e) => setBoatForm({ ...boatForm, flagState: e.target.value })} /></label><label>Home port<input value={boatForm.homePort} onChange={(e) => setBoatForm({ ...boatForm, homePort: e.target.value })} /></label><label>Owner<input value={boatForm.owner} onChange={(e) => setBoatForm({ ...boatForm, owner: e.target.value })} /></label><label>Dimensions<input value={boatForm.dimensions} onChange={(e) => setBoatForm({ ...boatForm, dimensions: e.target.value })} /></label><label>Manufacturer<input value={boatForm.manufacturer} onChange={(e) => setBoatForm({ ...boatForm, manufacturer: e.target.value })} /></label><label>MMSI<input value={boatForm.mmsi} onChange={(e) => setBoatForm({ ...boatForm, mmsi: e.target.value })} /></label><label>Engine<input value={boatForm.engine} onChange={(e) => setBoatForm({ ...boatForm, engine: e.target.value })} /></label><label className="wide-field">Safety<textarea value={boatForm.safety} onChange={(e) => setBoatForm({ ...boatForm, safety: e.target.value })} /></label><div className="inline-edit-actions"><button type="submit">{editingBoatId ? "Save boat" : "Create boat"}</button><button type="button" className="ghost-button" onClick={cancelBoatEdit}>Cancel</button></div></form> : <><div className="card-title-row"><h3>Boat manager</h3><div className="table-actions"><button type="button" className="edit-chip" onClick={() => startEditingBoat(activeBoat)}>Edit active boat</button><button type="button" className="edit-chip" onClick={() => { setEditingBoatId(null); setBoatForm(defaultBoatForm); setShowBoatManager(true); }}>New boat</button></div></div><dl><div><dt>Name</dt><dd>{activeBoat.name}</dd></div><div><dt>Type</dt><dd>{activeBoat.type}</dd></div><div><dt>Registration</dt><dd>{activeBoat.registration || "—"}</dd></div><div><dt>Flag / home port</dt><dd>{activeBoat.flagState || "—"} · {activeBoat.homePort || "—"}</dd></div><div><dt>Owner</dt><dd>{activeBoat.owner || "—"}</dd></div><div><dt>Ship data</dt><dd>{activeBoat.dimensions || "—"}</dd></div></dl></>}</article><article className="yacht-card"><div><p className="eyebrow">Yacht data</p><h3>Boat master data</h3></div><dl className="yacht-data-grid">{yachtDataOrder.map((label) => <div key={label}><dt>{label}</dt><dd>{activeBoat.yachtData[label] || "—"}</dd></div>)}</dl></article></div></section>}

        {activeModule === "crew" && <section className="sheet-detail module-panel"><div className="bottom-grid"><article className="info-card"><h3>Crew manager</h3><ul className="stack-list">{activeSheet.crew.map((person) => <li key={person.name}><strong>{person.name}</strong><span>{person.nationality} · {person.role}</span><small>{person.embarkation} → {person.disembarkation}</small></li>)}</ul></article><article className="info-card"><h3>Watch & daily checks</h3><ul className="check-list">{[...activeSheet.watchPlan, ...activeSheet.technicalChecks].map((item) => <li key={item}>{item}</li>)}</ul></article></div></section>}

        {activeModule === "compliance" && <section className="sheet-detail module-panel"><article className="compliance-card"><div><p className="eyebrow">Swiss compliance checklist</p><h3>Built from Hochseeausweis logbook requirements</h3></div><ul>{legalRequirements.map((requirement) => <li key={requirement}>{requirement}</li>)}</ul></article><article className="signature-card"><div><span>Logbook lead</span><strong>{activeSheet.skipper.name}</strong></div><div><span>Skipper</span><strong>{activeSheet.skipper.name}</strong></div><div><span>Digital personal-log status</span><strong>{activeSheet.status}</strong></div></article></section>}
      </section>
    </main>
  );
}
