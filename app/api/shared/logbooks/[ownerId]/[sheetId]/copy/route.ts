...
function validateOptions(value: unknown) {
  ...
  const allowed = new Set(["destinationBoatId", "includeCrew", "includePicture", "allowDuplicate"]);
  ...
  if (input.allowDuplicate !== undefined && typeof input.allowDuplicate !== "boolean") throw new SyntaxError("allowDuplicate must be a boolean.");
  return { destinationBoatId: input.destinationBoatId, ...(input.includeCrew === undefined ? {} : { includeCrew: input.includeCrew }), ...(input.includePicture === undefined ? {} : { includePicture: input.includePicture }), ...(input.allowDuplicate === undefined ? {} : { allowDuplicate: input.allowDuplicate }) };
}
