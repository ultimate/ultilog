...
export const DELETE = (request: Request, context: Context) => authenticatedMutation(request, async ownerId => { const { id, lineId } = await context.params; return deleteLogLine(id, lineId, await deleteRevision(request), ownerId); });
